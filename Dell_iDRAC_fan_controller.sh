...
IDRAC_LOGIN_STRING=$(set_iDRAC_login_string "$IDRAC_HOST" "$IDRAC_USERNAME" "$IDRAC_PASSWORD")
...
while true; do
  ...
  # If CPU 2 temperature sensor is present, check if it is overheating then apply Dell default dynamic fan control profile if true
    ...
    if [ $TEMPERATURE_INTERPOLATION_ACTIVATION_RANGE -gt 0 ]; then
      FAN_VALUE_TO_ADD=$(((DECIMAL_HIGH_FAN_SPEED_OBJECTIVE - DECIMAL_LOW_FAN_SPEED_OBJECTIVE) * (HIGHEST_CPU_TEMPERATURE - CPU_TEMPERATURE_THRESHOLD_FOR_FAN_SPEED_INTERPOLATION) / TEMPERATURE_INTERPOLATION_ACTIVATION_RANGE))
      DECIMAL_FAN_SPEED_TO_APPLY=$((DECIMAL_LOW_FAN_SPEED_OBJECTIVE + FAN_VALUE_TO_ADD))
    else
